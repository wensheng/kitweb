use crate::browser::BrowserAudioEnv;
use crate::ffmpeg::*;
...
impl AudioRuntime {
    pub fn start(
        ...
        let sink_name = format!("kitweb_{}", std::process::id());
        ...
        // Snap-confined browsers (chromium, etc.) connect to PulseAudio only
        // via the host unix socket; their launcher hardcodes
        // `PULSE_SERVER=unix:$XDG_RUNTIME_DIR/../pulse/native` whenever that
        // file exists. The minimal default.pa from kitim/docs/ssh-linux.md
        // omits `module-native-protocol-unix`, so the socket file is present
        // but the daemon has no module to service it and connections hang.
        // Make sure the module is loaded on whichever PA we're using.
    ...
    pub fn browser_audio_env(&self) -> BrowserAudioEnv<'_> {
        BrowserAudioEnv {
...
fn run_audio_capture(
    ...
    let stream = device
        .build_output_stream(
            ...
            move |err| {
                if std::env::var_os("KITWEB_CHILD_LOGS").is_some() {
                    eprintln!("kitweb: audio stream error: {}", err);
                }
            },
            None,
        )
...
unsafe fn capture_pulse_monitor<P: Producer<Item = f32>>(
    ...
    let v_stream_name = CString::new("kitweb audio capture").unwrap();
...
fn pulse_server_candidates(override_server: Option<&str>) -> Vec<Option<String>> {
    ...
    // Default local Pulse TCP port. Useful when the local unix socket is broken
    // but the daemon still serves over TCP (the SSH-from-Mac setup in
    // kitim/docs/ssh-linux.md enables this).
