...
pub fn run_capture(
    ...
    unsafe {
        ...
        let mut rgba_buf: Vec<u8> = vec![0u8; rgba_stride * src_height as usize];
        ...
        while running.load(Ordering::SeqCst) {
            ...
            loop {
                ...
                let frame_data = rgba_buf.clone();
                if tx.try_send(CaptureMsg::Frame {
                    rgba: frame_data,
                    width: src_width,
                    height: src_height,
                }).is_err() {
                    // Render thread is full; drop frame
                }
            }
        }
